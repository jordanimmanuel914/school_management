...
static CMyCheckCombiBox *m_pComboBox = 0;


BEGIN_MESSAGE_MAP(CMyCheckCombiBox, CComboBox)
	//{{AFX_MSG_MAP(CMyCheckCombiBox)
...
CMyCheckCombiBox::CMyCheckCombiBox()
{
	m_hListBox       = 0;
...
BOOL CMyCheckCombiBox::Create(DWORD dwStyle, const RECT& rect, CWnd* pParentWnd, UINT nID)
{
...
LRESULT CMyCheckCombiBox::OnCtlColorListBox(WPARAM wParam, LPARAM lParam) 
{
...
void CMyCheckCombiBox::DrawItem(LPDRAWITEMSTRUCT lpDrawItemStruct) 
{
...
void CMyCheckCombiBox::MeasureItem(LPMEASUREITEMSTRUCT lpMeasureItemStruct) 
{
	CClientDC dc(this);
...
// there may be many CMyCheckCombiBox windows active
//
void CMyCheckCombiBox::OnDropDown() 
{
	m_pComboBox = this;
...
//
void CMyCheckCombiBox::SelectAll(BOOL bCheck)
{
	INT nCount = GetCount();
...
//
LRESULT CMyCheckCombiBox::OnGetText(WPARAM wParam, LPARAM lParam)
{
...
//
LRESULT CMyCheckCombiBox::OnGetTextLength(WPARAM, LPARAM)
{
	// Make sure the text is updated
	RecalcText();
	return m_strText.GetLength();
...
//
void CMyCheckCombiBox::RecalcText()
{
	if (!m_bTextUpdated) {
		CString strText;
...
INT CMyCheckCombiBox::SetCheck(INT nIndex, BOOL bFlag)
{
...
BOOL CMyCheckCombiBox::GetCheck(INT nIndex)
{
